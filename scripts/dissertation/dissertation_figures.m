...
r_plot_training_curves(cdata, ndata, ts, [3.1]);
set(gcf, 'Name', 'similarity');

%
%r_plot_training_curves(cdata, ndata, ts, [0.3]); set(gcf, 'Name', 'ringo_expt1_classification_error');
%r_plot_training_curves(cdata, ndata, ts, [0.4]); set(gcf, 'Name', 'ringo_expt1_sumsquared_error');

%
composite_figure = figure('Name', 'expt1_inter_matters', 'Position', [84          97        1271         687]);
ax = subplot(2,3,1); r_plot_training_curves(cdata, ndata, ts, [0.4]); title(''); xlabel(''); mfe_copyaxes(gca, ax, true); close(gcf);
ax = subplot(2,3,2); r_plot_training_curves(cdata, ndata, ts, [0.6]); title(''); xlabel(''); ylabel(''); mfe_copyaxes(gca, ax, true); close(gcf);
ax = subplot(2,3,3); r_plot_training_curves(cdata, ndata, ts, [0.8]); title(''); xlabel(''); ylabel(''); mfe_copyaxes(gca, ax, true); close(gcf);
ax = subplot(2,3,4); r_plot_training_curves(cdata, ndata, ts, [1.4]); title(''); mfe_copyaxes(gca, ax, true); close(gcf);
ax = subplot(2,3,5); r_plot_training_curves(cdata, ndata, ts, [1.6]); title(''); ylabel('');  mfe_copyaxes(gca, ax, true); close(gcf);
ax = subplot(2,3,6); r_plot_training_curves(cdata, ndata, ts, [1.8]); title(''); ylabel('');  mfe_copyaxes(gca, ax, true); close(gcf);
