...
%
    ...
    if ~exist('r_plot_training_curves','file'), addpath(genpath(fullfile(fileparts(which(mfilename)), '..','..','code'))); end;
    ...
    for d1=1:length(datasets)
        ...
        r_plot_training_curves(cdata, ndata, ts, [0.4]);

        fprintf('%s\n', datasets{d1});
        for fignum=[0.6 0.8 1.4 1.6 2.2 3.2 3.3]
            %continue;
            r_plot_training_curves(cdata, ndata, ts, fignum, cache_file);
            %[~,~,oh] = legend();
...
%
%         for fignum=[0.4]
%             r_plot_training_curves(cdata, ndata, ts, fignum);
%             [~,~,oh] = legend();
...
%     end;
    ...
    for d1=1:length(datasets)
        ...
        for d2=d1+1:length(datasets)
            ...
            for fi=1:2
                ca = subplot(1,2,fi);
                r_plot_training_curves(cdata, ndata, ts, figs(fi));
