function [in_pats, out_pats, pat_cls, pat_lbls, idx] = r_pats_parity_vs_shift(sets, opt)
    ...
    in_pats = [ p_in_pats(:, 1:end/2)       -1*ones(p_npats, 1) s_in_pats(:, 1:end/2)        1*ones(s_npats, 1);
                s_in_pats(:, (1+end/2):end)  1*ones(s_npats, 1) p_in_pats(:, (1+end/2):end) -1*ones(p_npats, 1)];

    out_pats = [p_out_pats(:, 1:end/2)       s_out_pats(:, 1:end/2);
                s_out_pats(:, (1+end/2):end) p_out_pats(:, (1+end/2):end)];
